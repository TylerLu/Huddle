...
namespace Huddle.MetricWebApp.SharePoint
{
    public class QueryService
    {
        ...
        {
            var issueFilter = string.Format(@"<And>
                    <And>
                        <Eq>
                            <FieldRef Name='{0}' />
                ...
                </And>", SPLists.Issues.Columns.State, state, SPLists.Issues.Columns.TeamId, teamId,SPLists.Issues.Columns.Title,key);
            ...
            var metricFilter = string.Format(@"
                    ...
                    </Contains>", SPLists.Metrics.Columns.Title, key);
            ...
            var reasonFilter = string.Format(@"
                    <Contains>
                        <FieldRef Name='{0}' />
                        <Value Type='Text'>{1}</Value>
                    </Contains>", SPLists.Reasons.Columns.Title, key);
                ...
                if (reasonArray.Length > 0)
                {
                    ...
                    var relatedMetricIdsFilter = string.Format(@"
                                                <In>
                                                    ...
                                                    + @"</Values>
                                                </In>", SPLists.Metrics.Columns.ID, key);
                    var filter = string.Format(@"<And>"
                        +relatedMetricIdsFilter
                        + @"<Eq>
                            <FieldRef Name='{0}' />
                            <Value Type='Choice'>{1}</Value>
                        </Eq>
                    </And>", SPLists.Issues.Columns.State,state);
                    relatedMetricQuery.ViewXml = string.Format("<View><Query><Where>{0}</Where></Query></View>", filter);
